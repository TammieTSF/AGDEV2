...
static const int theMaxNumberOfProjectiles = 10;
	...
	, CurrentEmptyProjectile(0)
{
	...
	Boundary_BottomRight = Vector3(-300.0f, -50.0f, -300.0f);
}
...
void CProjectileManager::RemoveProjectile(const int index)
{
	if (IsValid(index))
	{
		std::cout << theListOfProjectiles[index]->GetPosition() << std::endl;
...
void CProjectileManager::Update(const double dt)
{
	for (int i = 0; i<theMaxNumberOfProjectiles; i++)
	{
		...
		//	RemoveProjectile(i);
		//}
		if (CheckThisPosition.x > Boundary_TopLeft.x || CheckThisPosition.x < Boundary_BottomRight.x ||
			CheckThisPosition.y > Boundary_TopLeft.y || CheckThisPosition.y < Boundary_BottomRight.y ||
			CheckThisPosition.z > Boundary_TopLeft.z || CheckThisPosition.z < Boundary_BottomRight.z)
		{
			RemoveProjectile(i);
		}
	}
